...
setTimeout(() => {
  document.querySelectorAll("button.plus").forEach((ele) => {
    ...
    ele.addEventListener("click", (event) => {
      document.getElementById("asterisk").innerText = Number(document.getElementById("asterisk").innerText) + 1;
      
      if(localStorage.getItem(ele.value)){
        localStorage.setItem(ele.value, Number(localStorage.getItem(ele.value))+1);
      }else{
